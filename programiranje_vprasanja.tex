...
\noindent OCaml optimizira repne klice. \\

\noindent Python repnih klicev namenoma ne optimizira, da so sporočila
...
Polimorfne funkcijo so opisane že zgoraj pod prvim vprašanjem. 
Gre za funkcije, katerih tip je v bistvu $\alpha$ oz. $'a$. \\

\noindent Parametrični tipi so tipi, ki nastopajo v polimorfnih 
funkcijah. Primer takega tipa je \textit{'a list}. Primer polimorfne 
funkcije s parametričnimi tipi je \texttt{fun 'a $\rightarrow$ 'b list $\rightarrow$ 'c list}.
Ta funkcija sprejme poljuben element in seznam poljubnih elementov, 
ki po tipu niso nujni enaki prvemu elementu in vrne seznam elementov
nekega tipa. 
...
\noindent Funkcije nam ob dani spremenljivki vrnejo določene vrednosti. Če te vrednosti uporabimo naprej v drugi funkciji, govorimo o \textit{funkcijah višjega reda}. Torej funkcija višjega reda je vsaka 
...
\noindent Če funkciji, ki sprejme dva argumenta podamo samo en argument,
dobimo novo funkcijo. Če mi to funkcijo poimenujemo, jo lahko
(kasneje) uporabimo na drugem argumentu. \\

\noindent V OCamlu je navada, da pišemo \textit{fun x $\rightarrow$ fun(x)}. Operator $\rightarrow$ v tem zapisu je desno asociativen.
...
\noindent Imamo funkcijo dveh argumentov, ki torej sprejme prvi argument,
sprejme drugega in nekaj naredi. Če ji podamo samo prvi element,
dobimo novo funkcijo (ki čaka na drug argument). \\

\noindent To vidimo tudi kadar imamo funkcijo $f$, ki sprejme dva argumenta $x$ in $y$.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=75mm]{slike/curryiranje.png}
    \label{fig:curryiranje}
\end{figure}
\subsection{Vsote in induktivni tipi}
Tip vsota je sestavljen iz posameznih kosov. Vrednosti tega
tipa naštejemo. Vsako vrednost napišemo z veliko začetnico in jih
ločimo z navpičnimi črtami. Tem vrednostim pravimo \textit{variante}.
Vsaka varianta ima svoj \textit{konstruktor} in ti konstruktorji
lahko sprejmejo tudi argumente. \\

\begin{figure}[h!]
    \centering
    \includegraphics[width=75mm]{slike/konstruktorji_in_variante.png}
    \label{fig:konstruktorji in variante}
\end{figure}

\noindent V zgornjem primeru je vsota sestavljena iz treh variant (Tocka,
Krog, Pravokotnik), Pravokotnik kot konstruktor pa sprejme
dva floata. \\

\noindent Delno definirane funkcije so funkcije, kjer ne naštejemo vseh variant.
Zanje je najvarneje, da uporabimo tip \textit{option}. Če ga uporabimo 
nas tipi prisilijo, da obravnavamo robne primere. \footnote{PREVERI, KAJ 
SO INDUKTIVNI TIPI}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=75mm]{slike/racunska_zahtevnost_graf.png}
...
Časovna zahtevnost je v odvisnoti od dolžine levega. \footnote{Dela rekurzivno
...
\noindent \underline{leva rotacija}
\begin{figure}[h!]
    \centering
...
Podatkovne strukture so dveh tipov in sicer lahko so \textit{spremenljive}
ali pa \textit{nespremenljive}. Glavna razlika med tema tipoma je
v tem, da lahko spremenljive strukture spremenimo "na mestu", 
kar pomeni, da nam ni potrebno na novo generirati cele strukture, 
če ji želimo dodati ali odvzeti element. Pri nespremenljivih strukturah
moramo to storiti. Ali je podatkovna struktura spremenljiva ali
ne, je odvisno od tega, kako je shranjena v pomnilniku. \\

\noindent Primer nespremenljive strukture v OCamlu je seznam, primer nespremenljive 
pa tabela in hash tabela. 
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=75mm]{slike/prikaz_veriznih_seznamov.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=75mm]{slike/pomnilnik_nov_seznam.png}
...
\noindent Tabele imajo tip \textit{array}. Tabele so v pomnilniku shranjene
tako, da je na prvem mestu velikost tabele, na naslednjih
pa zaporedni členi tabele. Razlika v zapisu seznama in tabele je v
tem, da elemente tabele vstavimo med $|\quad|$. Tabele v OCamlu
imajo fiksno dolžino. Tip \textit{array} je spremenljiv in elemente
spreminjamo z uporabo $\leftarrow$. Funkcije lahko vrnejo novo
tabelo ali pa obstoječo spremenijo na mestu.\\

\begin{figure}[h!]
    \centering
    \includegraphics[width=75mm]{slike/tabele.png}
    \label{fig:tabele}
\end{figure}

\noindent V Pythonu so seznami predstavljeni tako: \\

\begin{figure}[h!]
    \centering
    \includegraphics[width=75mm]{slike/Python_list.png}
    \label{fig:Python list}
\end{figure}
...
Temu, da rešitve podnalog sestavljajo rešitev celote, pravimo 
\textit{lastnost optimalne podstrukture}. \\

\noindent \underline{urejanje z zlivanjem} \\
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=75mm]{slike/urejanje_z_zlivanjem.png}
    \label{fig:urejanje z zlivanjem}
\end{figure}

\noindent\underline{hitro urejanje} \\
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=75mm]{slike/hitro_urejanje.png}
    \label{fig:hitro urejanje}
\end{figure} 
...
Fisher - Yatesov algoritem generira naključno permutacijo na 
končnem seznamu, tabeli, itd. Z njim v bistvu premešamo elemente. \\

\noindent Deluje tako, da začnemo na koncu (torej na $n$ - tem mestu) in si
naključno izberemo indeks $k_{0}$, ki je nekje med $0$ in $n - 1$.
Zamenjamo elementa na $k_{0}$ - tem in $n$ - tem mestu. Postopek ponovimo
tako, da določimo element na $n - 1$. mestu in si naključno izberemo
indeks $k_{1}$, ki je nekje med $0$ in $n - 2$. Postopek ponavljamo,
dokler ne pridemo do konca. Na čisto zadnjem koraku imamo tako določen
indeks $1$ in element s tem indeksom zamenjamo s tistim na začetnem mestu, 
saj druge izbire nimamo. \footnote{PREMISLI, ČE ŽELIŠ NAŠTETI
ŠE DRUGE METODE.} \\

\noindent Časovna zahtevnost Fisher - Yatesovega algoritma je 
$O(n)$, saj je $n - 1$ korakov in na vsakem samo zamenjamo člena.
...
Obravnava je že zgoraj v podpoglavju \textit{Metoda deli in 
vladaj}. \footnote{PREMISLI, ČE JO RES DOBRO RAZUMEŠ.}
\subsection{Dinamično programiranje in memoizacija}
Razlika med dinamičnim programiranjem in metodo deli in vladaj je 
v tem, da pri dinamičnem programiranju problema ne moremo
razbiti v podprobleme, ki bi  bili za faktor manjši, 
rešuje nas pa to, da se problemi prekrivajo, kar pomeni,
da isti problem rešujemo večkrat. \\

\noindent Ponavljanju problemov se izognemo na dva načina
\begin{itemize}
    \item rešitve pripravimo v ustreznem vrstnem redu \textit{(dinamično programiranje)}
    \item rešitve si samodejno shranjujemo \textit{(memoizacija)}
\end{itemize}

\noindent\underline{dinamično programiranje}
\begin{itemize}
    \item rešitve podnalog sestavljajo rešitev celote
    \item podnaloge se \textit{prekrivajo}
\end{itemize}

\noindent\underline{memoizacija}\footnote{\textit{memoizacija = "rešitve is samodejno shranjujemo"}} \\

\noindent Stvari si zapomnimo v seznam, kjer so ključi argumenti,
vrednosti pa pripadajoče rešitve problemov pri danem 
argumentu. Slovarji so implementirani z zgoščevalnimi tabelami, katerih
časovna zahtevnost je konstantna. Enako je s shranjevanjem elementov
v slovar. V Pythonu lahko funkcijo, ki memoizira napišemo sami, 
je pa že vključena v knjižnjici \textit{functools}. Do nje 
dostopamo tako, da zgoraj v datoteko napišemo \textit{from 
functools import lru\_cache}, v vrstico nad funkcijo pa moramo posebej
napisati $@$\textit{lru\_cache(maxsize=None)}. \\

\noindent\textit{lru\_cache} deluje tako, da pogleda, če je ključ že v
slovarju. Če ga ni, si ga shrani. Do problemov pride, če si za 
argumente vzamemo sezname, saj seznamov ne moremo shraniti v 
slovar. Tega problema se lahko rešimo s pomožno funkcijo. \\

Za memoizacijo v OCamlu ne uporabimo slovarjev, ampak \textit{zgoščevalne
tabele}. 
\end{document}